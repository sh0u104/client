#include "Misc.h"
#include "StageUVShader.h"

StageUVShader::StageUVShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\StageUVVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\StageUVPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void StageUVShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
...
// 描画
void StageUVShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void StageUVShader::End(ID3D11DeviceContext* dc)
{
